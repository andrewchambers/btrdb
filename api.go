...
// Like RawValues, but avoids channel sending overhead where possible.
func (s *Stream) ForEachRawValue(ctx context.Context, start int64, end int64, version uint64, f func(RawPoint)) (uint64, error) {
	var ep *Endpoint
	var err error
	for s.b.TestEpError(ep, err) {
		ep, err = s.b.ReadEndpointFor(ctx, s.uuid)
		...
		ver, err := ep.ForEachRawValue(ctx, s.uuid, start, end, version, f)
		s.b.TestEpError(ep, err)
		return ver, err
	}
	if err == nil {
